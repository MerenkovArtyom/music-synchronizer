...
- открой `.app`;
- на первом запуске приложение покажет welcome screen и переведёт в `Настройки`;
- вставь `Yandex Music token`;
- выбери `Obsidian vault path`;
- сохрани настройки;
- во время сохранения приложение проверит токен Яндекс Музыки перед записью `config.env`;
- после этого можно запускать `sync` без терминала.

Ожидаемые desktop-состояния:

- если не заполнены и токен, и `vault`, приложение показывает welcome screen с шагами настройки;
- если не хватает только токена или только `vault`, приложение показывает точный empty state и ведёт в `Настройки`;
- если токен невалидный, настройки не сохраняются, а пользователь получает сообщение, что токен нужно заменить;
- если API Яндекс Музыки временно недоступен, приложение предлагает повторить попытку позже;
- если в dev-режиме не найден `uv`, `python` или сам backend, desktop shell показывает отдельную ошибку окружения разработчика.

Важно для `npm run dev`: Electron overlay читает не только `config.env`, но и переменные окружения процесса. Если в терминале уже выставлены `YANDEX_MUSIC_TOKEN` или `OBSIDIAN_VAULT_PATH`, first-run экран не появится даже при пустом пользовательском config-файле.
...
- desktop UI валидирует токен Яндекс Музыки во время сохранения и не записывает нерабочий токен в `config.env`.
