...
- `uv run music-sync list --tag "tag"` — вывести активные сохранённые треки с указанным тегом
...
Также в заметке сохраняется `monthly_listens` и строка `Monthly listens (30d)` в теле файла. Значение вычисляется по истории прослушивания Яндекс Музыки за последние 30 дней.
